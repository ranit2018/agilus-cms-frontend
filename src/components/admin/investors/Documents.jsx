...
const setPDF = (refObj) => (cell, row) => {
    ...
    return (
        ...
        <LinkWithTooltip
            ...
            href="#"
            clicked={(e) => refObj.imageModalShowHandler(e, row.url)}
            id="tooltip-1"
        >
            {row.file}
        </LinkWithTooltip>
...
class Documents extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            activePage: 1,
            selectType: [
                { value: "1", label: "Composite Scheme of Arrangement And Amalgamation" },
                { value: "2", label: "Annual Reports" },
                { value: "3", label: " Download Content" },
                { value: "4", label: "Financials of Subsidiaries" },
            ],
            selectStatus: [
    ...
    getDocumets = (page = 1) => {
        ...
        API.get(
            `/api/investor/document?page=${page}&type=${encodeURIComponent(type)}&display_name=${encodeURIComponent(display_name)}&status=${encodeURIComponent(status)}`
        )
            .then((res) => {
                this.setState({
    ...
    documentSearch = (e) => {
        ...
        API.get(`/api/investor/document?page=1&type=${encodeURIComponent(type)}&display_name=${encodeURIComponent(display_name)}&status=${encodeURIComponent(status)}`)
            .then((res) => {
                this.setState({
                    documentList: res.data.data,
                    totalCount: res.data.count,
    ...
    deleteDocument = (id) => {
        API.delete(`/api/investor/document/${id}`)
            .then((res) => {
                swal({
                    closeOnClickOutside: false,
                    title: "Success",
    ...
    getDocumentDetails(id) {
        API.get(`/api/investor/document/${Number(id)}`)
            .then((res) => {
                this.setState({
                    showModal: true,
    ...
    handleSubmitEvent =  (values, actions) => {
        ...
        if (this.state.documentId > 0) {
            url = `/api/investor/document/${this.state.documentId}`;
            method = 'PUT';
        } else {
            url = `/api/investor/document/`;
            method = 'POST';
        }
    ...
    chageStatus = (cell, status) => {
        API.put(`/api/investor/document/change_status/${cell}`, { status: status == 1 ? String(0) : String(1) })
            .then((res) => {
                swal({
                    closeOnClickOutside: false,
                    title: "Success",
    ...
    fileChangedHandler = (event, setFieldTouched, setFieldValue, setErrors) => {
        ...
        if (event.target.files[0] && SUPPORTED_FORMATS.includes(event.target.files[0].type)) {
            ...
            //Unsupported
            setErrors({ file: "Only files with the following extensions are allowed: pdf" }); //Not working- So Added validation in "yup"
            this.setState({
                file: "",
                isValidFile: false,
            });
    ...
    modalCloseHandler = () => {
        this.setState({ documentDetails: {}, documentId: "", showModal: false, file: "" })
    };
    ...
    render() {
        ...
        const newInitialValues = Object.assign(initialValues, {
            type: documentDetails.type ? documentDetails.type : '',
            name: documentDetails.name ? documentDetails.name : '',
            file: '',
            status: documentDetails.status || +documentDetails.status === 0
                        ...
                        <div className="box">
                            <div className="box-body">
                                            ...
                                            return (
                                                <Form>
                                                    ...
                                                    <Modal.Header closeButton>
                                                        <Modal.Title>
                                                            {this.state.documentId > 0 ? 'Edit Document' : 'Add Document'}
                                                        </Modal.Title>
                                                    </Modal.Header>
                                                    <Modal.Body>
                                                            ...
                                                            <Row>
                                                                <Col xs={12} sm={12} md={12}>
                                                                    <div className="form-group">
                                                                        <label>
                                                                            Choose File
                                                                            <br /> <i>(Image type should be .pdf and maximum file size is 20 mb.)</i>
                                ...
                                >
                                    ...
                                    <Modal.Body>
                                        <center>
                                            <object data={this.state.url} width="100%" height="650" hspace="0" vspace="0"></object>
                                        </center>
                                    </Modal.Body>
                                </Modal>
                            </div>
