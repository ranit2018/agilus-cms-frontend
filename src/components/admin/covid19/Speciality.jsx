...
const setBlogImage = (refObj) => (cell, row) => {
    ...
    return (
        <img src={row.speciality_image} alt="Image" width="60" height="60" onClick={(e) => refObj.imageModalShowHandler(row.speciality_image)}></img>
    );
};
class Speciality extends Component {
    ...
    getSpecialityList = (page = 1) => {


        API.get(
            `/api/covid/speciality?page=${page}`
        )
            .then((res) => {
                this.setState({
                    specialityList: res.data.data,
    ...
    deleteSpeciality = (id) => {
        API.delete(`/api/covid/speciality/${id}`)
            .then((res) => {
                swal({
                    closeOnClickOutside: false,
                    title: "Success",
                    ...
                    this.getSpecialityList(this.state.activePage);
                });
            })
            .catch((err) => {
    ...
    getSpecialityDetails(id) {
        API.get(`/api/covid/speciality/${id}`)
            .then((res) => {
                this.setState({
                    showModal: true,
                    specialityDetails: res.data.data[0],
                    specialityId: id,
    ...
    handleSubmitEvent = async (values, actions) => {
        ...
        if (this.state.file) {
            if (this.state.file.size > FILE_SIZE) {
                ...
                const specialityDimension = getResolution("speciality");
                console.log("height", height);
                console.log("width", width);
                if (height != specialityDimension.height || width != specialityDimension.width) {
        ...
        if (this.state.specialityId > 0) {
            url = `/api/covid/speciality/${this.state.specialityId}`;
            method = 'PUT';
        } else {
            url = `/api/covid/speciality`;
            method = 'POST';
        }
       if (err_count == 0) {
        API({
    ...
    modalCloseHandler = () => {
        this.setState({ specialityDetails: {}, specialityId: "", showModal: false })
    };

    modalShowHandler = (event, id) => {
        event.preventDefault();
        this.setState({
            validationMessage: generateResolutionText('speciality'),
            fileValidationMessage: FILE_VALIDATION_MASSAGE
        })
        if (id) {
            this.getSpecialityDetails(id)
        } else {
            this.setState({ specialityDetails: {}, specialityId: "", showModal: true });
        }
    }
    ...
    chageStatus = (cell, status) => {
        API.put(`/api/covid/speciality/change_status/${cell}`, { status: status == 1 ? String(0) : String(1) })
            .then((res) => {
                swal({
                    closeOnClickOutside: false,
                    title: "Success",
    ...
    render() {
        ...
        const { specialityDetails } = this.state;

        const newInitialValues = Object.assign(initialValues, {
            file: '',
            title: specialityDetails.title ? htmlDecode(specialityDetails.title) : '',
            content: specialityDetails.content ? htmlDecode(specialityDetails.content) : '',
            status: specialityDetails.status || +specialityDetails.status === 0
                ? specialityDetails.status.toString()
                : ""
        });
                ...
                <small />
                            ...
                            <div className="col-lg-12 col-sm-12 col-xs-12  topSearchSection">
                                <div className="">
                                    ...
                                    >
                                        <i className="fas fa-plus m-r-5" /> Add Speciality
                    </button>
                                </div>
                    ...
                    <section className="content">
                        <div className="box">
                            <div className="box-body">
                                ...
                                <BootstrapTable data={this.state.specialityList}>
                                    <TableHeaderColumn
                                        isKey
                                        dataField="title"
                                            ...
                                            return (
                                                <Form>
                                                    ...
                                                    <Modal.Header closeButton>
                                                        <Modal.Title>
                                                            {this.state.specialityId > 0 ? 'Edit Speciality' : 'Add Speciality'}
                                                        </Modal.Title>
                                                    </Modal.Header>
                                                    <Modal.Body>
