...
class FAQ extends Component {
        ...
        API.delete(`/api/covid/faq/${id}`)
            .then((res) => {
                swal({
                    ...
                    this.getFAQList(this.state.activePage);
                });
            })
            .catch((err) => {
    ...
    getFAQdetails(id) {
        API.get(`/api/covid/faq/${id}`)
            .then((res) => {
                this.setState({
                    showModal: true,
                    faqDetails: res.data.data[0],
                    faqId: id,
    ...
    chageStatus = (cell, status) => {
        API.put(`/api/covid/faq/change_status/${cell}`, {status: status == 1 ? String(0) : String(1)})
        .then((res) => {
          swal({
            closeOnClickOutside: false,
            title: "Success",
    ...
    handleSubmitEvent = (values, actions) => {
        let url = '';
        let method = '';
        if (this.state.faqId > 0) {
            url = `/api/covid/faq/${this.state.faqId}`;
            method = 'PUT';
        } else {
            url = `/api/covid/faq`;
            method = 'POST';
        ...
        API({
            method: method,
            url: url,
            data: { title: values.title, content: values.content, status: values.status }
        }).then((res) => {
    ...
    modalCloseHandler = () => {
        this.setState({ faqDetails: {}, faqId: "", showModal: false })
    };

    modalShowHandler = (event, id) => {
        event.preventDefault();
        if (id) {
            this.getFAQdetails(id)
        } else {
            this.setState({ faqDetails: {}, faqId: "", showModal: true });
        }
    }

    handlePageChange = (pageNumber) => {
        this.setState({ activePage: pageNumber });
        this.getFAQList(pageNumber > 0 ? pageNumber : 1);
    };
    ...
    render() {
        ...
        const newInitialValues = Object.assign(initialValues, {
            title: faqDetails.title ? faqDetails.title : '',
            content: faqDetails.content ? faqDetails.content : '',
            status: faqDetails.status || +faqDetails.status === 0
                    ...
                    <section className="content">
                        <div className="box">
                            <div className="box-body">
                                ...
                                <BootstrapTable data={this.state.faqList}>
                                    <TableHeaderColumn
                                        isKey
                                        dataField="title"
                                            ...
                                            return (
                                                <Form>
                                                    ...
                                                    <Modal.Header closeButton>
                                                        <Modal.Title>
                                                            {this.state.faqId > 0 ? 'Edit FAQ' : 'Add FAQ'}
                                                        </Modal.Title>
                                                    </Modal.Header>
                                                    <Modal.Body>
