...
export default class LifeAtSrl extends Component {
  ...
  getCategories = (page = 1) => {
    API.get(`/api/home/facts_about?page=${page}`)
      .then((res) => {
        this.setState({
  ...
  getCategorydetails(id) {
    API.get(`/api/home/facts_about/${Number(id)}`)
      .then((res) => {
        this.setState({
          showModal: true,
  ...
  deleteCategory = (id) => {
    API.post(`/api/home/facts_about/${id}`)
      .then((res) => {
        swal({
          closeOnClickOutside: false,
          title: "Success",
  ...
  chageStatus = (cell, status) => {
    API.put(`/api/home/facts_about/change_status/${cell}`, {
  ...
  setHealthBenefitImage = (refObj) => (cell, row) => {
    if (row.facts_about_file !== null) {
      return (
        <>
          <img
            src={row.facts_about_file}
            alt="Inside SRL Image"
            height="100"
            // onClick={(e) => refObj.imageModalShowHandler(row.facts_about_file)}
          ></img>
        </>
      );
    } else {
  ...
  handleSubmitEvent = async (values, actions) => {
    ...
    formData.append("facts_about_file", this.state.inside_srl_file);

    if (this.state.categoryId > 0) {
      url = `/api/home/facts_about/${this.state.categoryId}`;
      method = "PUT";
    } else {
      url = `/api/home/facts_about`;
      method = "POST";
    }
            ...
            <div className="row">
              ...
              <div className="col-lg-12 col-sm-12 col-xs-12  topSearchSection">
                <div className="">
                  ...
                  >
                    <i className="fas fa-plus m-r-5" /> Add Fact About Srl
                  </button>
                </div>
