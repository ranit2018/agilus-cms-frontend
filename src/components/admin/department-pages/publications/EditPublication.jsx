...
class EditPublication extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      publication_id: this.props.match.params.id,
      alldata: [],
      isLoading: true,
    };
  }
  ...
  componentDidMount() {
    this.getPublicationbyId(this.state.publication_id);
    this.setState({
    ...
    API.get(`/api/department/publication/${id}`)
      .then((res) => {
        this.setState({
          ...
          publication_id: res.data.data[0].id,
          isLoading: false,
        });
      })
      .catch((err) => {
  ...
  handleSubmitEvent = (values, actions) => {
    ...
    let url = `/api/department/publication/${this.props.match.params.id}`;
    let method = "PUT";
  ...
  render() {
    ...
    const newInitialValues = {
      publication_image: "",
      publication_heading: htmlDecode(alldata.publication_heading),
      short_name: htmlDecode(alldata.short_name),
      publication_description: htmlDecode(alldata.publication_description),
