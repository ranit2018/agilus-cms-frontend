...
class AddMember extends Component {
  ...
  handleSubmitEvent = (values, actions) => {
    ...
    formData.append("about", values.about_member);
    formData.append("designation", values.designation);
    formData.append("status", String(values.status));

    let url = `/api/feed/key-members`;
    let method = "POST";
    if (this.state.member_image.size > FILE_SIZE) {
      actions.setErrors({ member_image: FILE_VALIDATION_SIZE_ERROR_MASSAGE });
      ...
      getHeightWidth(this.state.member_image).then((dimension) => {
        ...
        if (height != offerDimension.height || width != offerDimension.width) {
          ...
          API({
            ...
            .then((res) => {
              this.setState({ showModal: false, member_image: "" });
              swal({
