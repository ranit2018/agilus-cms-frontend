...
const setPDF = (refObj) => (cell, row) => {   
    ...
    return (
        ...
        <LinkWithTooltip
            ...
            id="tooltip-1"
        >
            {capitalizeFirstLetter(row.title.replaceAll(' ',"_"))}{`.pdf`}
        </LinkWithTooltip>
    );
};
...
class RnDocuments extends Component {
    ...
    getDocumets = (page = 1) => {
        ...
        API.get(
            `/api/rnd/rnd_category_file?page=${page}&status=${encodeURIComponent(status)}`
        )
            .then((res) => {
                console.log(res.data.data)
                this.setState({
    ...
    documentSearch = (e) => {
        ...
        API.get(`/api/rnd/rnd_category_file?page=1&category_id=${encodeURIComponent(type)}&title=${encodeURIComponent(display_name)}&status=${encodeURIComponent(status)}`)
            .then((res) => {
                this.setState({
                    documentList: res.data.data,
                    totalCount: res.data.count,
    ...
    chageStatus = (cell, status) => {
        API.put(`/api/rnd/rnd_category_file/change_status/${cell}`, { status: status == 1 ? String(0) : String(1) })
            .then((res) => {
                swal({
                    closeOnClickOutside: false,
                    title: "Success",
                                                            ...
                                                            <Row>
                                                                <Col xs={12} sm={12} md={12}>
                                                                    <div className="form-group">
                                                                        <label>
                                                                        Category<span className="impField">*</span>
                                                                        </label>
                                                                        <Field
                                                                            name="type"
                                                                            component="select"
