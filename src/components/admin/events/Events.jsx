...
class Thumb extends React.Component {
    ...
    render() {
        ...
        return (
            ...
            <span>
                <i className="fa fa-window-close" aria-hidden="true" onClick={(e) => this.remove(file)} />
                <img src={thumb}
                    alt={file.name}
...
const setBannerImage = (refObj) => (cell, row) => {
    ...
    return (
        ...
        >
            <img src={cell} alt="Featured Image" width="60" height="60" onClick={(e) => refObj.imageModalShowHandler(row.featured_image)}></img>
...
class Events extends Component {
    ...
    getEventCategory = (page = 1) => {
        ...
        API.get(
            `/api/feed/get_category_by_medium/2`
        )
            .then((res) => {
                this.setState({
    ...
    getEventsList = (page = 1) => {
        ...
        API.get(
            `/api/events?page=${page}&event_title=${encodeURIComponent(event_title)}&event_location=${encodeURIComponent(event_location)}&event_type=${encodeURIComponent(event_type)}&status=${encodeURIComponent(status)}`
        )
            .then((res) => {
                console.log(res);
                this.setState({
    ...
    eventSearch = (e) => {
        ...
        if (event_title === "" && event_location === "" && event_type === "" && status === "") {
            return false;
        }

        API.get(`/api/events?page=1&event_title=${encodeURIComponent(event_title)}&event_location=${encodeURIComponent(event_location)}&event_type=${encodeURIComponent(event_type)}&status=${encodeURIComponent(status)}`)
            .then((res) => {
                this.setState({
                    events: res.data.data,
                    totalCount: res.data.count,
    ...
    chageStatus = (cell, status) => {
        API.put(`/api/events/change_status/${cell}`, { status: status == 1 ? String(0) : String(1) })
            .then((res) => {
                swal({
                    closeOnClickOutside: false,
                    title: "Success",
                                ...
                                <form className="form">
                                    ...
                                    <div className="">
                                        <input
                                            ...
                                            name="event_location"
                                            id="event_location"
                                            placeholder="Filter by Event Location"
                                        />
                                    </div>
